...
namespace XNBDecomp
{
	...
	public class LzxDecoderOld
	{
        ...
        public LzxDecoderOld(int window)
		{
			...
			m_state.main_elements = (ushort)(LzxConstantsOld.NUM_CHARS + (posn_slots << 3));
			m_state.header_read = 0;
			m_state.frames_read = 0;
			m_state.block_remaining = 0;
			m_state.block_type = LzxConstantsOld.BLOCKTYPE.INVALID;
			...
			m_state.PRETREE_table = new ushort[(1 << LzxConstantsOld.PRETREE_TABLEBITS) + (LzxConstantsOld.PRETREE_MAXSYMBOLS << 1)];
			m_state.PRETREE_len = new byte[LzxConstantsOld.PRETREE_MAXSYMBOLS + LzxConstantsOld.LENTABLE_SAFETY];
			m_state.MAINTREE_table = new ushort[(1 << LzxConstantsOld.MAINTREE_TABLEBITS) + (LzxConstantsOld.MAINTREE_MAXSYMBOLS << 1)];
			m_state.MAINTREE_len = new byte[LzxConstantsOld.MAINTREE_MAXSYMBOLS + LzxConstantsOld.LENTABLE_SAFETY];
			m_state.LENGTH_table = new ushort[(1 << LzxConstantsOld.LENGTH_TABLEBITS) + (LzxConstantsOld.LENGTH_MAXSYMBOLS << 1)];
			m_state.LENGTH_len = new byte[LzxConstantsOld.LENGTH_MAXSYMBOLS + LzxConstantsOld.LENTABLE_SAFETY];
			m_state.ALIGNED_table = new ushort[(1 << LzxConstantsOld.ALIGNED_TABLEBITS) + (LzxConstantsOld.ALIGNED_MAXSYMBOLS << 1)];
			m_state.ALIGNED_len = new byte[LzxConstantsOld.ALIGNED_MAXSYMBOLS + LzxConstantsOld.LENTABLE_SAFETY];
			/* initialise tables to 0 (because deltas will be applied to them) */
			for(int i = 0; i < LzxConstantsOld.MAINTREE_MAXSYMBOLS; i++) m_state.MAINTREE_len[i] = 0;
			for(int i = 0; i < LzxConstantsOld.LENGTH_MAXSYMBOLS; i++) m_state.LENGTH_len[i] = 0;
		}
		...
		public int Decompress(Stream inData, int inLen, Stream outData, int outLen)
		{
			...
			while(togo > 0)
			{
				...
				if(m_state.block_remaining == 0)
				{
					...
					if(m_state.block_type == LzxConstantsOld.BLOCKTYPE.UNCOMPRESSED) {
						if((m_state.block_length & 1) == 1) inData.ReadByte(); /* realign bitstream to word */
						bitbuf.InitBitStream();
					}
					
					m_state.block_type = (LzxConstantsOld.BLOCKTYPE)bitbuf.ReadBits(3);;
					...
					{
					case LzxConstantsOld.BLOCKTYPE.ALIGNED:
						for(i = 0, j = 0; i < 8; i++) { j = bitbuf.ReadBits(3); m_state.ALIGNED_len[i] = (byte)j; }
						MakeDecodeTable(LzxConstantsOld.ALIGNED_MAXSYMBOLS, LzxConstantsOld.ALIGNED_TABLEBITS,
						                m_state.ALIGNED_len, m_state.ALIGNED_table);
						/* rest of aligned header is same as verbatim */
						goto case LzxConstantsOld.BLOCKTYPE.VERBATIM;
						
					case LzxConstantsOld.BLOCKTYPE.VERBATIM:
						ReadLengths(m_state.MAINTREE_len, 0, 256, bitbuf);
						ReadLengths(m_state.MAINTREE_len, 256, m_state.main_elements, bitbuf);
						MakeDecodeTable(LzxConstantsOld.MAINTREE_MAXSYMBOLS, LzxConstantsOld.MAINTREE_TABLEBITS,
						                m_state.MAINTREE_len, m_state.MAINTREE_table);
						if(m_state.MAINTREE_len[0xE8] != 0) m_state.intel_started = 1;
						
						ReadLengths(m_state.LENGTH_len, 0, LzxConstantsOld.NUM_SECONDARY_LENGTHS, bitbuf);
						MakeDecodeTable(LzxConstantsOld.LENGTH_MAXSYMBOLS, LzxConstantsOld.LENGTH_TABLEBITS,
						                m_state.LENGTH_len, m_state.LENGTH_table);
						break;
						
					case LzxConstantsOld.BLOCKTYPE.UNCOMPRESSED:
					...
					case LzxConstantsOld.BLOCKTYPE.VERBATIM:
						while(this_run > 0)
						{
							main_element = (int)ReadHuffSym(m_state.MAINTREE_table, m_state.MAINTREE_len,
							                           LzxConstantsOld.MAINTREE_MAXSYMBOLS, LzxConstantsOld.MAINTREE_TABLEBITS,
							                           bitbuf);
							if(main_element < LzxConstantsOld.NUM_CHARS)
							{
							...
							else
							{
								...
								main_element -= LzxConstantsOld.NUM_CHARS;
								
								match_length = main_element & LzxConstantsOld.NUM_PRIMARY_LENGTHS;
								if(match_length == LzxConstantsOld.NUM_PRIMARY_LENGTHS)
								{
									length_footer = (int)ReadHuffSym(m_state.LENGTH_table, m_state.LENGTH_len,
									                            LzxConstantsOld.LENGTH_MAXSYMBOLS, LzxConstantsOld.LENGTH_TABLEBITS,
									                            bitbuf);
									match_length += length_footer;
								}
								match_length += LzxConstantsOld.MIN_MATCH;
								...
								if(match_offset > 2)
								{
									...
									if(extra > 3)
									{
										...
										aligned_bits = (int)ReadHuffSym(m_state.ALIGNED_table, m_state.ALIGNED_len,
										                           LzxConstantsOld.ALIGNED_MAXSYMBOLS, LzxConstantsOld.ALIGNED_TABLEBITS,
										                           bitbuf);
		...
		private void ReadLengths(byte[] lens, uint first, uint last, BitBuffer bitbuf)
		{
			...
			MakeDecodeTable(LzxConstantsOld.PRETREE_MAXSYMBOLS, LzxConstantsOld.PRETREE_TABLEBITS,
			                m_state.PRETREE_len, m_state.PRETREE_table);
			
			for(x = first; x < last;)
			{
				z = (int)ReadHuffSym(m_state.PRETREE_table, m_state.PRETREE_len,
				                LzxConstantsOld.PRETREE_MAXSYMBOLS, LzxConstantsOld.PRETREE_TABLEBITS, bitbuf);
				if(z == 17)
				{
		...
		struct LzxState {
			...
			public LzxConstantsOld.BLOCKTYPE	block_type;			/* type of this block						*/
