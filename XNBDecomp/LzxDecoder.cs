    ...
    public class LzxDecoder
    {
        ...
        private class BitBuffer
        {
            ...
            public void EnsureBits(byte bits)
            {
                ...
                {
                    byte lo = this.ReadByte();
                    byte hi = this.ReadByte();
                    this.bit_buffer |= (uint)(((hi << 8) | lo) << (sizeof(uint) * 8 - 16 - this.bits_left));
                    this.bits_left += 16;
                }
            }
            ...
            {
                return (this.bit_buffer >> ((sizeof(uint) * 8) - bits));
            }
            ...
            {
                byte lo = this.ReadByte();
                byte ml = this.ReadByte();
                byte mh = this.ReadByte();
                byte hi = this.ReadByte();
